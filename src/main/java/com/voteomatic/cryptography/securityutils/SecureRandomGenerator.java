...
/**
 * Interface for generating cryptographically secure random numbers and bytes. Defines a contract
 * for sources of randomness suitable for cryptographic operations like key generation, nonces, and
 * padding. Implementations should use a Cryptographically Secure Pseudo-Random Number Generator
 * (CSPRNG).
 */
public interface SecureRandomGenerator {
  ...
  /**
   * Generates a secure random BigInteger that is uniformly distributed in the range 0 to (limit -
   * 1), inclusive.
   *
  ...
  /**
   * Generates a secure random BigInteger with the specified number of bits. The probability that a
   * BigInteger constructed this way is prime is negligible, but it's useful for generating random
   * exponents or blinding factors.
   *
