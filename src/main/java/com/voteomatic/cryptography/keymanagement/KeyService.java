...
/**
 * Interface for managing cryptographic keys (generation, storage, retrieval). Defines the contract
 * for services responsible for the lifecycle of keys.
 */
public interface KeyService {
  ...
  /**
   * Stores a key pair, associating it with a unique identifier. The underlying implementation
   * should handle secure storage.
   *
   ...
   * @throws KeyManagementException if storing the key fails (e.g., identifier collision, storage
   *     error).
   */
  void storeKeyPair(KeyPair keyPair, String identifier, char[] password)
   ...
   * @throws KeyManagementException if the key pair is not found, the password is incorrect, or
   *     retrieval fails.
   */
  KeyPair retrieveKeyPair(String identifier, char[] password) throws KeyManagementException;

  /**
   * Retrieves only the public key associated with a given identifier. This is often needed without
  ...
  /**
   * Optional: Verifies the integrity or properties of a public key. This could involve checking if
   * parameters (p, g) are valid or if the key belongs to a specific group. Implementation details
