...
public interface KeyStorageHandler {
   ...
   */
  void storeKeyPair(String alias, KeyPair keyPair, Certificate certificate, char[] password)
   ...
   * @throws DataHandlingException if retrieving the key pair fails (e.g., alias not found,
   *     incorrect password).
   */
  KeyPair retrieveKeyPair(String alias, char[] password) throws DataHandlingException;

  /**
   * Retrieves only the public key associated with a given alias. This typically does not require a
   * password.
   *
