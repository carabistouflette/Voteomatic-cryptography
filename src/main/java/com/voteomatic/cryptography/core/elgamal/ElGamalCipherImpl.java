...
/**
 * Implements the ElGamal encryption and decryption scheme. This implementation uses
 * java.math.BigInteger for cryptographic operations.
 */
public class ElGamalCipherImpl implements ElGamalCipher {
   ...
   *
   * @param secureRandomGenerator The generator for creating the ephemeral secret k. Must not be
   *     null.
   ...
   * @param message The message to encrypt as a BigInteger. Must not be null. The message must be
   *     representable within the group defined by p.
   ...
   * @throws ArithmeticException if cryptographic computations encounter issues (e.g., invalid
   *     parameters).
   */
  @Override
  public EncryptionResult encrypt(PublicKey publicKey, BigInteger message) {
    ...
    if (pMinusTwo.compareTo(BigInteger.ZERO) <= 0) {
      throw new ArithmeticException(
          "Prime p must be greater than 3 for secure random generation range.");
    }
   ...
   * @throws ArithmeticException if the modular inverse does not exist or other calculation errors
   *     occur.
   */
  @Override
  public BigInteger decrypt(PrivateKey privateKey, Ciphertext ciphertext) {
    ...
    if (c1.compareTo(p) >= 0
        || c1.compareTo(BigInteger.ZERO) < 0
        || c2.compareTo(p) >= 0
        || c2.compareTo(BigInteger.ZERO) < 0) {
      throw new IllegalArgumentException(
          "Ciphertext components must be non-negative and less than p");
    }
    ...
    try {
      ...
      // If g^k mod p = 0, this implies p divides g^k, only possible if p divides g (invalid params)
      // or k=0 (not allowed).
      // Wrap the original exception if needed, but the basic constructor is sufficient here.
      throw new ArithmeticException(
          "Failed to compute modular inverse (s = "
              + s
              + ", p = "
              + p
              + "). Ciphertext might be invalid or parameters incorrect.");
    }
