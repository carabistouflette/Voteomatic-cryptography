...
class SchnorrProverTest {
  ...
  void setUp() {
    ...
    prover = SchnorrProver.create(mockHashAlgorithm, mockSecureRandomGenerator);
  ...
  void constructor_nullHashAlgorithm_throwsException() {
    IllegalArgumentException e =
        assertThrows(
            ...
            () -> {
              SchnorrProver.create(null, mockSecureRandomGenerator);
            });
  ...
  void constructor_nullRandomGenerator_throwsException() {
    IllegalArgumentException e =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              SchnorrProver.create(mockHashAlgorithm, null);
            });
  ...
  void generateProof_success() throws ZkpException, SecurityUtilException {
    ...
    BigInteger s =
        v.subtract(c.multiply(x)).mod(q); // (5 - 3*7) mod 11 = (5 - 21) mod 11 = -16 mod 11 = 6
  ...
  void generateProof_hashAlgorithmThrowsException() throws SecurityUtilException {
    ...
    assertEquals("Failed to compute challenge hash for Schnorr proof", e.getMessage());
