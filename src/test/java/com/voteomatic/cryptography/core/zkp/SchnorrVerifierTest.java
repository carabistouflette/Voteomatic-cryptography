...
class SchnorrVerifierTest {
    ...
    void setUp() {
        ...
        verifier = new SchnorrVerifier(mockHashAlgorithm);
        ...
        validProof = new SchnorrProof(new BigInteger("16"), new BigInteger("6"));
    }

    @Test
    void constructor_nullHashAlgorithm_throwsException() {
        IllegalArgumentException e = assertThrows(IllegalArgumentException.class, () -> {
            new SchnorrVerifier(null);
        });
    ...
    void verifyProof_tIsZero_returnsFalse() throws ZkpException {
        SchnorrProof proofWithZeroT = new SchnorrProof(BigInteger.ZERO, validProof.getS());
        assertFalse(verifier.verifyProof(statement, proofWithZeroT));
    }

    @Test
    void verifyProof_tEqualsP_returnsFalse() throws ZkpException {
        SchnorrProof proofWithTequalsP = new SchnorrProof(p, validProof.getS());
    ...
    void verifyProof_tGreaterThanP_returnsFalse() throws ZkpException {
        SchnorrProof proofWithTgreaterP = new SchnorrProof(p.add(BigInteger.ONE), validProof.getS());
    ...
    void verifyProof_sIsNegative_returnsFalse() throws ZkpException {
        SchnorrProof proofWithNegativeS = new SchnorrProof(validProof.getT(), BigInteger.valueOf(-1));
        assertFalse(verifier.verifyProof(statement, proofWithNegativeS));
    }

    @Test
    void verifyProof_sEqualsQ_returnsFalse() throws ZkpException {
        SchnorrProof proofWithSequalsQ = new SchnorrProof(validProof.getT(), q);
    ...
    void verifyProof_sGreaterThanQ_returnsFalse() throws ZkpException {
        SchnorrProof proofWithSgreaterQ = new SchnorrProof(validProof.getT(), q.add(BigInteger.ONE));
