...
public class ElGamalCipherImplTest {
  ...
  void setUp() {
    ...
    secureRandomGenerator = SecureRandomGeneratorImpl.create(new SecureRandom()); // Factory method
  ...
  void testEncryptDecrypt_Success() {
    ...
    Assertions.assertTrue(
        message.compareTo(BigInteger.ONE) >= 0 && message.compareTo(domainParams.getP()) < 0,
    ...
    Assertions.assertEquals(
        message, decryptedMessage, "Decrypted message should match the original message");
  }
  ...
  /**
   * Tests that decrypt throws NullPointerException when ciphertext component c1 is null. Requires
   * Ciphertext constructor to allow nulls (modification made).
   */
  @Test
  void testDecrypt_NullCiphertextC1() {
    // 4-space indent
    // Construct ciphertext with null c1 (possible after modifying Ciphertext constructor)
    Ciphertext ciphertextWithNullC1 =
        new Ciphertext(null, BigInteger.TEN); // c2 is arbitrary non-null
  ...
  /**
   * Tests that decrypt throws NullPointerException when ciphertext component c2 is null. Requires
   * Ciphertext constructor to allow nulls (modification made).
   */
  @Test
  void testDecrypt_NullCiphertextC2() {
    ...
    Ciphertext ciphertextWithNullC2 =
        new Ciphertext(BigInteger.TEN, null); // c1 is arbitrary non-null
