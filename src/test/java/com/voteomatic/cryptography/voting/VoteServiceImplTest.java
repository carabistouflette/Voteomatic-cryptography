...
class VoteServiceImplTest {
  ...
  @Mock private ElGamalCipher elGamalCipher;

  @Mock private KeyService keyService; // Although not directly used in methods, it's a dependency

  @Mock private ZkpProver prover; // Use generic interface

  @Mock private ZkpVerifier verifier; // Use generic interface

  @Mock private DomainParameters domainParameters;
  ...
  void setUp() {
    ...
    domainParams =
        new DomainParameters(p_val, g_val, q_val); // Keep real one for other tests if needed

    // Stub the mocked domainParameters needed by VoteServiceImpl constructor (called via
    // @InjectMocks)
    ...
    // Note: q is not directly used in the constructor's precomputation based on VoteServiceImpl
    // code.
    ...
    voteService =
        new VoteServiceImpl(domainParameters, elGamalCipher, keyService, prover, verifier);
  }
  ...
  void testCastVote_Success() throws VotingException, ZkpException { // Add ZkpException
    ...
    Ciphertext expectedCiphertext =
        new Ciphertext(BigInteger.valueOf(123), BigInteger.valueOf(456)); // Dummy ciphertext

    // Mock the encrypt call to return EncryptionResult
    EncryptionResult mockEncryptionResult =
        new EncryptionResult(expectedCiphertext, mockRandomness);
    when(elGamalCipher.encrypt(eq(samplePublicKey), eq(expectedMessage)))
        .thenReturn(mockEncryptionResult);

    // Mock the prover call
    when(prover.generateProof(
            any(DisjunctiveChaumPedersenStatement.class),
            any(DisjunctiveChaumPedersenWitness.class)))
    ...
    assertEquals(
        mockDcpProof,
        encryptedVote.getValidityProof().get(),
        "Proof should match the mocked proof");
  ...
  void testCastVote_Success_NoVote() throws VotingException, ZkpException { // Add ZkpException
    ...
    Ciphertext expectedCiphertext =
        new Ciphertext(BigInteger.valueOf(789), BigInteger.valueOf(101)); // Dummy ciphertext
  ...
  void testCastVote_InvalidVoteOption() {
    ...
    assertEquals(
        "Error casting vote: Invalid vote option: 'Maybe'. Only 'Yes' or 'No' are allowed.",
        thrown.getMessage());
  ...
  void testCastVote_EncryptionError()
      throws ZkpException { // Add ZkpException for verify(never()) signature check
    // Arrange
    ...
    RuntimeException encryptionException =
        new RuntimeException("Encryption failed"); // Or a more specific crypto exception if defined

    // Mock encrypt to throw exception
    when(elGamalCipher.encrypt(
            eq(samplePublicKey),
            any(BigInteger.class))) // Match any BigInteger message for simplicity here
    ...
    verify(prover, never())
        .generateProof(any(), any()); // Prover should not be called if encryption fails
  }
  ...
  void testTallyVotes_Success_Homomorphic() throws VotingException {
    ...
    Ciphertext ct1 =
        new Ciphertext(
            domainParams.getG().mod(domainParams.getP()),
            domainParams.getG().multiply(y).mod(domainParams.getP())); // Represents m1=g^1
    Ciphertext ct2 =
        new Ciphertext(
            domainParams.getG().mod(domainParams.getP()),
            y.mod(domainParams.getP())); // Represents m2=g^0=1
    ...
    Ciphertext expectedProductCiphertext =
        identity.multiply(ct1, domainParams.getP()).multiply(ct2, domainParams.getP());
    ...
    assertEquals(
        BigInteger.ONE, tallyResult, "Tally count 'k' should be 1 for one 'Yes' and one 'No' vote");
  ...
  void testTallyVotes_EmptyList_Homomorphic() throws VotingException {
    ...
    assertEquals(
        BigInteger.ZERO, tallyResult, "Tally of empty list should be BigInteger.ZERO (k=0)");
  ...
  void testTallyVotes_ListWithNullVote_Homomorphic() throws VotingException {
    ...
    Ciphertext ctYes =
        new Ciphertext(
            domainParams.getG().modPow(BigInteger.TWO, domainParams.getP()),
            domainParams
                .getG()
                .multiply(samplePublicKey.getY().modPow(BigInteger.TWO, domainParams.getP()))
                .mod(domainParams.getP())); // Enc(g^1)
  ...
  void testTallyVotes_ListWithVoteHavingNullCiphertext_Homomorphic() throws VotingException {
    ...
    Ciphertext ctNo =
        new Ciphertext(
            domainParams.getG().modPow(BigInteger.valueOf(3), domainParams.getP()),
            BigInteger.ONE
                .multiply(samplePublicKey.getY().modPow(BigInteger.valueOf(3), domainParams.getP()))
                .mod(domainParams.getP())); // Enc(g^0)
  ...
  void testTallyVotes_FinalDecryptionError_Homomorphic() {
    ...
    assertEquals(
        "Error during final tally decryption or interpretation: Final Decryption failed",
        thrown.getMessage());
    assertEquals(decryptionException, thrown.getCause());
    verify(elGamalCipher, times(1)).decrypt(eq(samplePrivateKey), eq(expectedProductCiphertext));
    verify(elGamalCipher, times(1))
        .decrypt(any(), any()); // Ensure only the final decryption was attempted
  }
  ...
  void testTallyVotes_CannotDetermineTally() {
    ...
    assertEquals(
        "Error during final tally decryption or interpretation: Could not determine the vote tally"
            + " (k) from the decrypted result. The result (10) was not found in the precomputed"
            + " map. It might be invalid or exceed the maximum precomputed tally of 10000.",
        thrown.getMessage());
    verify(elGamalCipher, times(1)).decrypt(eq(samplePrivateKey), eq(expectedProductCiphertext));
  }
  ...
  void testVerifyVote_NullProofParameter() throws VotingException, ZkpException {
    // Arrange
    EncryptedVote encryptedVoteWithProof =
        createEncryptedVoteWithMockProof(); // EV has a proof internally
  ...
  void testVerifyVote_VerifierThrowsZkpException() throws ZkpException {
    ...
    when(verifier.verifyProof(eq(mockDcpStatement), eq(mockDcpProof))).thenThrow(zkpException);
  ...
  void testVerifyVote_VerifierThrowsRuntimeException() throws ZkpException {
    ...
    when(verifier.verifyProof(eq(mockDcpStatement), eq(mockDcpProof))).thenThrow(runtimeException);
    ...
    assertEquals(
        "Unexpected error during vote verification: Unexpected verifier error",
        thrown.getMessage());
  ...
  // The current verifyVote implementation doesn't have explicit type checks before calling
  // verifyProof.
