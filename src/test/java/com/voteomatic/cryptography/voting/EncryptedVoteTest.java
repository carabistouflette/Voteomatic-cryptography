...
class EncryptedVoteTest {
  ...
  void constructorWithProof_NullProof_ShouldSucceed() {
    ...
    assertEquals(ciphertext1, vote.getVoteCiphertext());
    assertEquals(
        Optional.empty(), vote.getValidityProof(), "Proof should be absent if null is passed.");
  }
  ...
  void constructorWithoutProof_ValidInput_ShouldSucceed() {
    ...
    assertEquals(
        Optional.empty(),
        vote.getValidityProof(),
        "Proof should be absent when using the constructor without proof.");
  }
  ...
  void getValidityProof_WhenProofAbsent_ShouldReturnEmptyOptional() {
    ...
    assertFalse(
        vote1.getValidityProof().isPresent(), "Optional should be empty when proof is null.");
    assertFalse(
        vote2.getValidityProof().isPresent(),
        "Optional should be empty when using constructor without proof.");
  }
  ...
  void hashCode_EqualObjects_NullProofVsNoProofConstructor_ShouldHaveEqualHashCodes() {
    ...
    assertEquals(
        vote1.hashCode(),
        vote2.hashCode(),
        "Hashcodes for null proof and omitted proof should be equal.");
  }
  ...
  void toString_WithoutProof_ContainsProofAbsent() {
    ...
    assertTrue(
        str1.contains("validityProof=absent"),
        "toString should indicate proof is absent when null.");
    assertTrue(str1.startsWith("EncryptedVote{"));
    assertTrue(str1.endsWith("}"));

    assertTrue(str2.contains(ciphertext1.toString()));
    assertTrue(
        str2.contains("validityProof=absent"),
        "toString should indicate proof is absent when omitted.");
